...
const double P_fdPhi[8] = {1./17.21,1./17.21,1./17.21,1./17.21,
    ...
    #if RHICrun == 4                                 // from analysis note for ppg086 special setting for run 4 
        const double P_kDC_phi = 0.206;
        const double P_kRICH_phi = 0.309;        
        const double P_scale = 0.97;                 // for some data phi0 was not corrected for momentum scale difference of 0.97  
    #else                                            // for all other ++ field runs
        const double P_kDC_phi = (1-P_delta)*P_kDC;  
        const double P_kRICH_phi = P_kBfield;         
    ...
    // DC resolution
        ...
        const double sigma_eta_det   = 0.00068;  // PC1 resolution 
        const double sigma_phi_ms    = 0.0043;   // rad GeV - all material including all VTX layers ~ 15% x/X0
    #elif RHICrun == 4
        const double sigma_alpha_ms  = 0.0007;   // rad GeV
        const double sigma_alpha_det = 0.0011;   // rad
        const double sigma_eta_det   = 0.00068;  // PC1 resolution 
        const double sigma_phi_ms    = 0.00068;  // rad GeV - 0.4% x/X0 in run 4 with helium bag 
    #else 
        const double sigma_alpha_ms  = 0.00;   // rad GeV
        const double sigma_alpha_det = 0.00;   // rad
        const double sigma_eta_det   = 0.00;   // PC1 resolution 
        const double sigma_phi_ms    = 0.00;   // rad GeV - all material including all VTX layers ~ 15% x/X0
    #endif
