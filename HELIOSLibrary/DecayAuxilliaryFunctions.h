...
// mo   = other decay particle mass - default 0 for photon
{
  ...
  val = 2.*c1/mll *  pow(f,1.5) * sqrt(1.-4.*eps/q) * (1.+2.*eps/q);
  return val;
}
...
{ 
  ...
  Double_t Gamma = Gp*mp/mll * pow((mll*mll/4.-ml*ml)/(mp*mp/4.-ml),1.5); 

  value = sqrt(1.-4.*eps)*(1.+2.*eps)*pow((1.-4.*delta),1.5);
...
//
// Virtual photon
  double f_gee(Double_t *x, Double_t *p) {
  Double_t mll=x[0];
  Double_t y = KrollWada(mll,vphotonMass,eMass);
  return y; 
}
