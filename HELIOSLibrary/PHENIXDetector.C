...
////////////////////////////////////////////////////////////////////////
//
// Phi V calculation used tio identify conversion candidates
//
//
// Axel Drees 23/05/2025
//
Double_t PHENIXDetector::PhiV(TLorentzVector positron, TLorentzVector electron){

  Double_t pv=1;
  TVector3 z(0.0, 0.0, 1.0);                       // unit vector in z (beam) direction
  TVector3 p2 = positron.Vect().Unit();
  TVector3 p1 = electron.Vect().Unit();

  TVector3       p1p2  = p1+p2;                    // Compute the sum of the two vectors
  TVector3       u     = p1p2.Unit();              // Extract the 3-momentum and compute the unit vector
  TVector3       v     = p1.Cross(p2).Unit();      // vector perpendiculaar to p1-p2 plane
  TVector3       w     = u.Cross(v).Unit();        // orientation of opening angle
  TVector3       wc    = u.Cross(z).Unit();        // expected orientation of a conversion pair
...
// Axel Drees 5/21/2025  add distortion on phi for missing momentum scal correction in run 4
//
Double_t PHENIXDetector::SmearDCphi0(Double_t pt, Double_t phi0)
{
  ...
  phi = randy.Gaus(phi0,sigma_phi0) - q_charge *P_kDC/pt*(1-P_delta)*(1-P_scale);
...
TLorentzVector PHENIXDetector::ReconstructTrack(TLorentzVector VT, Int_t q)
{
  ...
  if (q != 0) {
    if (arm > 0){
      m    = VT.M();
      pt   = SmearPt(VT.Pt(), q);
      phi  = SmearDCphi0(pt, VT.Phi());
      eta  = SmearDCeta(pt, VT.Eta());
      Reco.SetPtEtaPhiM(pt,eta,phi,m);
