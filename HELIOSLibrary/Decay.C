...
//
  ...
  if (DecayType == "TwoBody") {
    if (DecayName == "omega->ee" or DecayName == "omega->mm" or DecayName == "rho0->ee" or DecayName == "rho0->mm"
        or DecayName == "phi->ee" or DecayName == "phi->mm" or DecayName == "g->ee") {
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////
bool Decay::TwoBodyDecay(TLorentzVector &parent, TLorentzVector &daughter1, TLorentzVector &daughter2){
//
...
//            7/30/2025  - recoded to avoid numerical issues causing mass-E1-E2 != 0 
//
  TRandom3 randy = TRandom3(0);                // Random Generator
  Double_t pi = 3.1415926536;                  // define pi

  Double_t px,py,pz,E1,E2,p;                   // define generic 4 vector component 
...
// decay in rest frame 
//
  E1 = (mass*mass - m2*m2 + m1*m1)/2/mass;      // energy of daughter 1
  E2 = mass - E1;                               // energy of daugther 2
  p  = sqrt((E1+m1)*(E1-m1));                   // momemtum in rest frame
  Double_t phi = randy.Uniform(0.,2.*pi);       // phi random from 0. to 2pi
  Double_t costheta = randy.Uniform(-1.,1.);    // generate cos(theta)
  Double_t sintheta = sqrt((1+costheta)*(1-costheta)); // calculate sin(theta)
  pz = p*costheta;                              // longitudinal momentum
  px = p*sintheta*cos(phi);                     // transverse momentum x component
  py = p*sintheta*sin(phi);                     // transverse momentum y component
  daughter1.SetPxPyPzE(px,py,pz,E1);            // set 4 vector of 1 decay particle
  daughter2.SetPxPyPzE(-px,-py,-pz,E2);         // set 4 vector of 2 decay particle in opposit direction
  if (mass-E1-E2 != 0) {
    if (debug) cout << "------------ bad energy balance " << mass-E1-E2 << endl;
    return false;
  }
  if (parent.P()>0.001) {
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////
bool Decay::TwoBodyDecayAnisotropic(TLorentzVector &parent, 
                                    TLorentzVector &daughter1, 
                                    TLorentzVector &daughter2,
                                    Double_t lambda=1.){
//
// Anisotropic two body decay of parent into two particles
// with a (1 + lambda Cos(theta)^2)
//
// input:  parent 4 vector
// output: daughter1 &2 4 vector 
//
// Axel Drees 7/31/2025  - addapted from TwoBodyDecay 
//
  TRandom3 randy = TRandom3(0);                // Random Generator
  Double_t pi = 3.1415926536;                  // define pi
  Double_t px,py,pz,E1,E2,p;                   // define generic 4 vector component 
...
//
  ...
  p  = sqrt((E1+m1)*(E1-m1));                   // momemtum in rest frame
  Double_t phi = randy.Uniform(0.,2.*pi);       // phi random from 0. to 2pi
  Double_t costheta;                            // generate cos(theta)
  do {costheta = (randy.Uniform(-1,1));}
  while ((1.0+lambda*costheta*costheta)<(randy.Uniform(0.,2.0)));
  Double_t sintheta = sqrt((1+costheta)*(1-costheta)); // calculate sin(theta)
  pz = p*costheta;                              // longitudinal momentum
  px = p*sintheta*cos(phi);                     // transverse momentum x component
  py = p*sintheta*sin(phi);                     // transverse momentum y component
  ...
  daughter2.SetPxPyPzE(-px,-py,-pz,E2);         // set 4 vector of 2 decay particle in opposit direction
  if (mass-E1-E2 != 0) {
    cout << "------------ bad energy balance " << mass-E1-E2 << endl;
    return false;
  }
  if (parent.P()>0.001) {
     daughter1.Boost(boost);                     // boost with parent momentum
...
//            7/31/2025        add anisotropy for gee decays (taken from exodus)
//
  TRandom3 randy = TRandom3(0);                       // Random Generator
  double pairMass=0;
  ...
  double m1 = decay1.M();
  double m2 = decay2.M();
  TLorentzVector leptonPair,leptonPairCM;
  TVector3 parentBoost = parent.BoostVector();        // boost vector of parent particle
//
// mass of ee pair
  TF1 *h = (TF1 *)gROOT->FindObject(DecayName);       // get leptonPair mass

  do { pairMass = h->GetRandom(); } while ((pairMass>=mass || pairMass<=m1+m2));

  leptonPairCM.SetPxPyPzE(0.,0.,0.,pairMass);       // set leptonPair lorentz vector
  leptonPair.SetPxPyPzE(0.,0.,0.,pairMass);         // set leptonPair lorentz vector
  if(debug) std::cout << "pair mass: generated " << pairMass;      

// decay parent->decay3 leptonPair
   TwoBodyDecay(parent,leptonPair,decay3);

//
// decay lepton pair first, decay1, decay 2 are in the rest fram of the leptonpair
  if (decay3.M()<0.01) {
     double beta_square = 1.0 - 4.0*(m1*m1)/(pairMass*pairMass);
     double lambda      = beta_square/(2.0-beta_square);
     TwoBodyDecayAnisotropic(leptonPairCM,decay1,decay2,lambda);
  }else{
     TwoBodyDecay(leptonPairCM,decay1,decay2);
  }
//   
  if(debug) std::cout << " \t mass after decay " << (decay1+decay2).M() << " \t pair pt  " << (decay1+decay2).Pt() << std::endl;       
//
// rotate and boost decay1 and decay 2
  TVector3 boost = leptonPair.BoostVector();      // boost vector of lepton pair parent particle
  TVector3 direction = leptonPair.Vect().Unit();  // Normalize to get direction of lepton pair
  TVector3 zAxis(0, 0, 1);                        // Default z-axis
  TVector3 rotationAxis = zAxis.Cross(direction); // Axis to rotate around
  Double_t angle = zAxis.Angle(direction);        // Angle between z-axis and direction
  TVector3 rotated = decay1.Vect();               // rotate decay1
  rotated.Rotate(angle, rotationAxis);
  decay1.SetVect(rotated);
  decay1.Boost(boost);
  rotated = decay2.Vect();
  rotated.Rotate(angle, rotationAxis);
  decay2.SetVect(rotated);
  decay2.Boost(boost);
 
  if(debug) std::cout << " \t mass after boost " << (decay1+decay2).M() << " \t pair pt  " << (decay1+decay2).Pt() << std::endl;       
  if(debug) parent.Print();
  if(debug) (decay1+decay2+decay3).Print();                   
  if(debug) decay3.Print();                   
  if(debug) decay1.Print();
  if(debug) decay2.Print();
  if(debug) leptonPair.Print(); 
  if(debug) (decay1+decay2).Print();      
}
/////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
//   
  ...
  } else if (DecayName == "g->ee") {
      TF1 *h_Mass = new TF1(DecayName,f_gee,2.*eMass,vphotonMass); 
