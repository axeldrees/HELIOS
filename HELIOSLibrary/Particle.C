...
//
  ...
  Particle::Particle(TString n){        // sets particle properties for known particles    
    ...
    if (name == "pi0"){                     // neutral pion
      ...
      stable = true;
      SetPxPyPzE(0.,0.,0.,0.);
    } else if (name == "vphoton") {          // virtual gamma
      id = vphotonID;
      charge = 0;
      weight = 1.;
      mass = 0;
...
// Generate particle with flat pt from pt_low to pt_high
//                               phi over 2pi
//                               rapidity from y_low to y_high
//
// input  Double_t pt_low      - lower pt bound 
//        Double_t pt_high     - upper pt bound
//        Double_t y_low       - lower y bound  
//        Double_t y_high      - upper y bound  
// 
// sets 4 vector of particle
//
// Axel Drees    2/11/2026 
//
void Particle::GenerateP(Double_t pt_low, Double_t pt_high, Double_t y_low, Double_t y_high, Bool_t rap=true) {
   Double_t pt,phi,rapidity,eta; 
   pt        = randy.Uniform(pt_low,pt_high);
   phi       = randy.Uniform(0.,2*pi);
...
// preset lorentz vector of possible decay particles
  ...
  if (name == "pi0") {                             
    ...
    } else if (name == "vphoton") {
     if (debug) std::cout << " definging decay of virtual photon'" << std::endl;

     DecayBranch[0].SetType("TwoBody");                  // g->ee 
     DecayBranch[0].SetName("g->ee");                    // define name of decay
     DecayBranch[0].SetMassDistributions();              // set virtual photon mass distribution for "rho-ee"
     DecayBranch[0].SetNumberOfDecayParticles(2);        // number of daughters  
     DecayBranch[0].SetBR(BR_g_ee);
     ...
     if (debug) std::cout << " defineing decay of phi" << std::endl;
