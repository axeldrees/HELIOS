...
//
  ...
  Double_t value = pt*2*3.141*A* pow( exp(-a*mt-b*mt*mt)+mt/p0 ,n);
  return value;
}
...
const int GPR_etapi_n=500;
const double GPR_etapi_ptmax = 1.477;  // log10(pt in GeV/c)
const double GPR_etapi_ptmin = -1.8;    // log10(pt in GeV/c)
const double GPR_etapi_gap = (GPR_etapi_ptmax-GPR_etapi_ptmin)/GPR_etapi_n;

double GPR_pi_pp200_pt[GPR_pi_pp200_n], GPR_pi_pp200_yield[GPR_pi_pp200_n], GPR_pi_pp200_yield_error[GPR_pi_pp200_n];
double GPR_etapi_pt[GPR_etapi_n], GPR_etapi[GPR_etapi_n], GPR_etapi_error[GPR_etapi_n];
double GPR_etapi_pt_0020[GPR_etapi_n], GPR_etapi_0020[GPR_etapi_n], GPR_etapi_error_0020[GPR_etapi_n];
double GPR_etapi_pt_2060[GPR_etapi_n], GPR_etapi_2060[GPR_etapi_n], GPR_etapi_error_2060[GPR_etapi_n];
...
//  std::cout<<"trash: "<<trashline<<std::endl;  
  getline(infile,trashline); 
//  std::cout<<"trash: "<<trashline<<std::endl;  
  for(int i = 0; i < GPR_etapi_n; i++){
    infile >> GPR_etapi_pt[i] >> GPR_etapi[i] >> GPR_etapi_error[i];
...
double Weight_GPR_etapi_uni(double pt, double opt=0){
  ...
  if (lpt>=GPR_etapi_ptmax) return 0;
  if (lpt<=GPR_etapi_ptmin) return GPR_etapi[0]+opt*GPR_etapi_error[0];
  gap = GPR_etapi_gap; 
  target_index = (lpt-GPR_etapi_ptmin)/gap;
  index = floor(target_index);
  alpha = target_index - index;

  if (index < GPR_etapi_n-1) {
     value = GPR_etapi[index]*(1-alpha) + GPR_etapi[index+1]*alpha; 
     error = GPR_etapi_error[index]*(1-alpha) + GPR_etapi_error[index+1]*alpha;
  }  
  return value+opt*error;
}
//////////////////////  AuAu 20-60%
void Read_GPR_etapi_2060(){
  std::string trashline;
  std::ifstream infile;
  infile.open(GPR_etapi_AuAu_2060);
  getline(infile,trashline); 
  cout << trashline << endl;
  getline(infile,trashline); 
  for(int i = 0; i < GPR_etapi_n; i++){
    infile >> GPR_etapi_pt_2060[i] >> GPR_etapi_2060[i] >> GPR_etapi_error_2060[i];
  }
  infile.close();
}
double Weight_GPR_etapi_2060(double pt, double opt=0){
  double gap, lpt, value=0, error=0, target_index, alpha;
  int index;
  ...
  if (lpt>=GPR_etapi_ptmax) return 0;
  if (lpt<=GPR_etapi_ptmin) return GPR_etapi_2060[0]+opt*GPR_etapi_error_2060[0];
  gap = GPR_etapi_gap; 
  target_index = (lpt-GPR_etapi_ptmin)/gap;
  index = floor(target_index);
  alpha = target_index - index;
  ...
  if (index < GPR_etapi_n-1) {
     value = GPR_etapi_2060[index]*(1-alpha) + GPR_etapi_2060[index+1]*alpha; 
     error = GPR_etapi_error_2060[index]*(1-alpha) + GPR_etapi_error_2060[index+1]*alpha;
  }  
  return value+opt*error;
}
//////////////////////  AuAu 00-20%
void Read_GPR_etapi_0020(){
    std::string trashline;
  std::ifstream infile;
  infile.open(GPR_etapi_AuAu_0020);
  getline(infile,trashline); 
  getline(infile,trashline); 
  for(int i = 0; i < GPR_etapi_n; i++){
    infile >> GPR_etapi_pt_0020[i] >> GPR_etapi_0020[i] >> GPR_etapi_error_0020[i];
  }
  infile.close();
}
double Weight_GPR_etapi_0020(double pt, double opt=0){
  double gap, lpt, value=0, error=0, target_index, alpha;
  int index;
  ...
  if (lpt>=GPR_etapi_ptmax) return 0;
  if (lpt<=GPR_etapi_ptmin) return GPR_etapi_0020[0]+opt*GPR_etapi_error_0020[0];
  gap = GPR_etapi_gap; 
  target_index = (lpt-GPR_etapi_ptmin)/gap;
  index = floor(target_index);
  alpha = target_index - index;
  ...
  if (index < GPR_etapi_n-1) {
     value = GPR_etapi_0020[index]*(1-alpha) + GPR_etapi_0020[index+1]*alpha; 
     error = GPR_etapi_error_0020[index]*(1-alpha) + GPR_etapi_error_0020[index+1]*alpha;
  }  
  return value+opt*error;
}
...
TGraphErrors* Omega2Pi(const char* file){

  std::ifstream infile(file);  
  const int nPred=1000;
  double x_gpr[nPred], y_gpr[nPred], e_gpr[nPred];

  for(int i = 0; i < nPred; i++){
    infile >> x_gpr[i] >> y_gpr[i] >> e_gpr[i];
    cout << x_gpr[i] << "  " << y_gpr[i] << "  " << e_gpr[i] << endl; 
   }
  TGraphErrors* gpr = new TGraphErrors(1000, x_gpr, y_gpr, 0, e_gpr);
  return gpr;

}

TGraphErrors* Eta2Pi(const char* file){
